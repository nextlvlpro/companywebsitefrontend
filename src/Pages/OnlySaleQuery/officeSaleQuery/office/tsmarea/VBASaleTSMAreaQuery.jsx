...
export default function VBASaleTSMAreaQuery() {
    ...
    <div className='p-2 mt-4 flex flex-col items-center justify-center'>
        ...
        {tsmarealist?.map((items,i)=> (
            <div key={i} className='flex flex-col items-center justify-center w-full gap-2'>
                <Link to={'/tsmareavbasalequery/'+items.tsmArea} className='p-2 bg-blue-500 w-[80px] text-center text-white mt-1 rounded'>
                    {items.tsmArea}
                </Link>
            </div>
        ))}
